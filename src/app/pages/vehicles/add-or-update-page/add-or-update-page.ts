...
@Component({
  selector: "add-or-update-page-vehicles",
  templateUrl: "add-or-update-page.html",
  styleUrls: ["./add-or-update-page.scss"],
})
export class AddOrUpdateVehiclesPage {
  ...
  getVehicle() {
    ...
    const sub = this.route.params.subscribe(async (params) => {
      ...
      if (params && params.id) {
        this.dataToEdit = (
          await this.service.getVehicle(this.route.snapshot.params.id)
        ).data();
        console.clear();
  ...
  async save(apartmentForm: NgForm, user: User) {
    if (apartmentForm.valid) {
      const id = this.loader.show();
      try {
        await this.service.addVehicle(apartmentForm.value, user.uid);
        this.loader.hide(id);
        apartmentForm.resetForm({});
        this.router.navigate(["/app/tabs/vehicles"]);
      } catch (error) {
  ...
  async update(apartmentForm: NgForm, user: User, docId: string) {
    if (apartmentForm.valid) {
      const id = this.loader.show();
      try {
        await this.service.updateVehicle(apartmentForm.value, docId, user.uid);
        this.loader.hide(id);
        apartmentForm.resetForm({});
        this.router.navigate(["/app/tabs/vehicles"]);
      } catch (error) {
