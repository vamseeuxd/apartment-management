...
export class AddOrUpdateRulesPage {
  ...
  getRule() {
    ...
    const sub = this.route.params.subscribe(async (params) => {
      ...
      if (params && params.id) {
        this.dataToEdit = (
          await this.service.getRule(this.route.snapshot.params.id)
        ).data();
        console.clear();
  ...
  async save(apartmentForm: NgForm, user: User) {
    if (apartmentForm.valid) {
      const id = this.loader.show();
      try {
        await this.service.addRule(apartmentForm.value, user.uid);
        this.loader.hide(id);
        apartmentForm.resetForm({});
        this.router.navigate(["/app/tabs/rules"]);
      } catch (error) {
  ...
  async update(apartmentForm: NgForm, user: User, docId: string) {
    if (apartmentForm.valid) {
      const id = this.loader.show();
      try {
        await this.service.updateRule(apartmentForm.value, docId, user.uid);
        this.loader.hide(id);
        apartmentForm.resetForm({});
        this.router.navigate(["/app/tabs/rules"]);
      } catch (error) {
